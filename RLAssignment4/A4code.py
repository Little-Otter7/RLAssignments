...
def train_double_dqn(env_name, episodes=100, gamma=0.99, epsilon_start=1.0, epsilon_end=0.1, epsilon_decay=200):
    env = gym.make(env_name)
    state_dim = env.observation_space.shape[0]
...
def evaluate_algorithm(env_name, train_func, episodes):
    total_rewards = []
    for _ in range(episodes):
        rewards = train_func(env_name)  # Assuming train_func trains for one episode and returns the total reward
        total_rewards.append(rewards)
    return total_rewards

def plot_performance(dqn_rewards, double_dqn_rewards):
    plt.figure(figsize=(10, 5))
    plt.plot(dqn_rewards, label='DQN', color='blue')
    plt.plot(double_dqn_rewards, label='Double DQN', color='red')
    plt.title('Comparison of DQN and Double DQN')
    plt.xlabel('Episodes')
    plt.ylabel('Total Reward')
    plt.legend()
    plt.show()

# Main setup to run the experiments
